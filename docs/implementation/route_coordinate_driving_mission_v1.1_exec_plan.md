...
| `Makefile` | modified | M8 follow-up: `scenarionet-v1-2-rebuild` now copies `scenario_catalog.parquet` to `scenario_catalog_driving_mission_v1_1_1.parquet` right after `build_runtime_databases`, so evaluation's `MISSION_CATALOG_FILENAME` fallback is reproducible from a clean host instead of relying on an ad hoc manual copy |
| `src/thesis_rl/rulebook/v2/components/progress.py` | modified | pre-merge reconciliation bugfix: `evaluate_progress`'s `MissionSnapshot` type annotation was only visible via a function-local import, tripping `ruff` `F821`; added a `TYPE_CHECKING`-guarded module-level import (static-analysis-only defect, inert at runtime under `from __future__ import annotations`, no behavioral regression test required) |
| `tests/test_hydra_preset_run_configs.py` | modified | pre-merge reconciliation bugfix: `test_final_scalar_pipeline_defaults_compose` still asserted the superseded `bounded_satisfaction_rank` default; updated to the `ADR-057`/`DEC-SCAL11-001`-approved `bounded_priority_weighted_rank` default, matching `conf/scalarization/default.yaml` (already correct) |
| `tests/test_rulebook_v2_transition.py` | modified | pre-merge reconciliation bugfix: `_snapshot()` test helper and one manual `MissionSnapshot` construction never set `s_m`, required by R4 (`AC-RCM-004`); added `s_m=x`/`s_m=7.0` matching each call's route position |
| `tests/test_rulebook_synthetic_scenarios.py` | modified | pre-merge reconciliation bugfix: added `_attach_synthetic_mission_snapshot()` helper so the two live-transition tests wrap `LiveSnapshotAdapter`-captured snapshots with a route-projected `MissionSnapshot.s_m`, mirroring production's `ThesisScenarioEnv` wiring pattern |
| `src/thesis_rl/rulebook/v2/components/progress.py` | modified | pre-merge reconciliation bugfix: restored the `REQ-EF-15`/M6a `route_outside_fraction`/`route_adherence` diagnostic, dropped (dead code) by the R4 mission-v1.1 rewrite (commit `4ce1233`); `evaluate_progress` gained optional `ego_footprint`/`task_corridor` keyword parameters (default `None`, preserving every existing caller); diagnostic-only, no cost/margin change |
| `src/thesis_rl/rulebook/v2/transition.py` | modified | pre-merge reconciliation bugfix: restored `"ego_footprint"`/`"task_corridor"` (via the previously-orphaned `_task_corridor(cache)`) in the `"progress"` entry of `component_inputs`, re-wiring the `route_outside_fraction`/`route_adherence` diagnostic inputs dropped by the same rewrite |
| `tests/test_run_metadata.py` | modified | pre-merge reconciliation bugfix: stale `evaluation_protocol.version == "1.0"` literal updated to `"1.2"` (current `AUTHORITATIVE` `EVAL-PROTOCOL` version per `ADR-042`); production `metadata.py` was already correct |
| `src/thesis_rl/envs/thesis_scenario_env.py` | modified | pre-merge reconciliation bugfix (user-approved restoration, "vai dai ripristinalo"): `reward_function` substitutes the dense `step_reward` for MetaDrive's flat `success_reward` whenever the native `_is_arrive_destination` predicate disagrees with the mission-tracker-owned `_is_thesis_success`, restoring `success_reward_suppressed` diagnostics dropped by commit `4ce1233` |
...
| `make smoke-gpu` re-run (visual confirmation of the corrected GIF overlays) | PASS | user-run, 2026-08-06; confirmed the coordinate-frame fix resolves the previously reported GIF symptoms end to end |
| `scenarionet-v1-2-rebuild`: `scenario_catalog_driving_mission_v1_1_1.parquet` production | RESOLVED | 2026-08-06. The ad hoc host-filesystem copy noted above is no longer needed: `scenarionet-v1-2-rebuild` (`Makefile`) now copies `scenario_catalog.parquet` to `scenario_catalog_driving_mission_v1_1_1.parquet` (the filename `MISSION_CATALOG_FILENAME` in `src/thesis_rl/envs/factory.py` falls back to when `env.catalog_path` is not set explicitly) immediately after `build_runtime_databases`, via the `dataset-pipeline` service. `make -n scenarionet-v1-2-rebuild` confirmed the expanded recipe includes the copy; ran it for real against the host dataset -- exit 0, `md5sum` of both files identical. `factory.py` and every consumer of `env.catalog_path`/`dataset_root` are unchanged: the runtime still reads from the same `catalog/` directory as always, only the pipeline now reproducibly populates both filenames. Reproducible from a clean host: any future `scenarionet-v1-2-rebuild` (direct or via `scenarionet-v1-2-rebuild-existing`) regenerates it automatically. |
| `make lint` (pre-merge reconciliation) | FAIL then PASS | 2026-08-08. Found `F821 Undefined name MissionSnapshot` in `src/thesis_rl/rulebook/v2/components/progress.py:44` (`evaluate_progress` signature referenced the type only via a function-local import, invisible to static analysis under `from __future__ import annotations`). Fixed with a `TYPE_CHECKING`-guarded module-level import; the pre-existing function-local import is retained for the runtime `isinstance` check. Static-analysis-only defect (inert at runtime, so no behavioral regression test required); `make lint` now PASS. |
| `make format-check` (scoped, `progress.py`) | FAIL then PASS | 2026-08-08. Reformatted with `make format PYTHON_QUALITY_PATHS=src/thesis_rl/rulebook/v2/components/progress.py` per the materially-modified-file rule; scoped `make format-check` now PASS. |
| `make test` (pre-merge reconciliation, full suite, first pass) | 1300 passed, 47 failed | 2026-08-08. Count matched the `47 pre-existing unrelated failures` baseline already recorded on this branch by the scalarization v1.1 ExecPlan (commit `6f70195`). 20/47 individually cross-referenced against prior sessions' documented pre-existing baselines (`test_rulebook_v2_transition.py` 14, `test_thesis_scenario_env.py::test_thesis_reward_suppresses_native_short_route_bonus` 1, `test_eval_artifacts.py` 4, `test_run_metadata.py` 1). The remaining 27 were not individually named before, so a targeted differential was run against `scenarionet-implementation` (via `git worktree add` + submodule init, same 4 test files) rather than accepting the count match alone. |
| differential re-run of the 27 uncross-referenced failures against `scenarionet-implementation` | 26/27 pre-existing, 1 genuine stale-test regression found | 2026-08-08. `tests/test_rulebook_synthetic_scenarios.py` (22 of 27) and `tests/test_async_evaluation.py::test_renderables_keep_completed_episode_count_after_job_finishes` (1 of 27) reproduce identically on `scenarionet-implementation` (in fact 18 additional `reset_and_step_in_scenario_online_env` parametrizations that fail on the parent now *pass* on this branch, an improvement from the mission work). `tests/test_build_named_panel_manifest_script.py` (2 of 27) reproduce identically. `tests/test_hydra_preset_run_configs.py::test_run_profile_tune_overrides_experiment_budget` (1 of 27) reproduces identically. But `tests/test_hydra_preset_run_configs.py::test_final_scalar_pipeline_defaults_compose` passed on `scenarionet-implementation` and failed here -- a genuine difference, not a false positive from the count-match heuristic. |
| root cause and fix for `test_final_scalar_pipeline_defaults_compose` | FIXED | 2026-08-08. Not a production defect: `conf/scalarization/default.yaml` was correctly updated to the `ADR-057`/`DEC-SCAL11-001=A` approved default (`bounded_priority_weighted_rank`) by the scalarization v1.1 implementation (commit `6f70195`, already on this branch), but this one test's hardcoded default-mode assertion in `tests/test_hydra_preset_run_configs.py:335` was never updated to match, so it still asserted the superseded `bounded_satisfaction_rank`. Fixed by updating the assertion to the approved default, with a comment citing `ADR-057`/`DEC-SCAL11-001`; this is realigning a stale test to an already-approved contract change, not weakening a mandatory test. Scoped `make lint`/`make format-check` PASS on the modified test file. |
| `make test` (pre-merge reconciliation, full suite, second pass after the stale-test fix) | 1301 passed, 46 failed | 2026-08-08. Exactly one fewer failure than the first pass, confirming the fix and no new regressions. All 46 remaining failures are now individually confirmed pre-existing and unrelated (20 previously cross-referenced across prior sessions plus 26 confirmed by this session's differential re-run against `scenarionet-implementation`). Worktree removed after use; no artifact left behind. |
| root cause and fix, `test_rulebook_v2_transition.py` (14 of the 46) | FIXED | 2026-08-08. All 14 shared one root cause: `evaluate_progress` (R4, `AC-RCM-004`) requires `MissionSnapshot.s_m` (exact canonical route station) to be non-`None`, but the file's `_snapshot()` test helper constructed every `MissionSnapshot` positionally without ever setting `s_m`, and one additional call site (line ~906) built a `MissionSnapshot` manually with the same gap. Test-fixture-only defect, not a production regression: every affected test's route is a straight line along `x` from `(0,0)`, so `s_m=x` is the geometrically correct station. Fixed by adding `s_m=x` to `_snapshot()` and `s_m=7.0` to the one manual construction (matching that call's `position_xy=(7.0, 0.0)`). All 20 tests in the file now pass (was 6 passed/14 failed). Scoped `make lint`/`make format-check` PASS. |
| root cause and fix, `tests/test_rulebook_synthetic_scenarios.py` (22 of the 46) | FIXED | 2026-08-08. Same underlying contract (R4 requires exact `s_m`), different mechanism: `LiveSnapshotAdapter`/`capture_env_snapshot` (`src/thesis_rl/rulebook/v2/context/live_adapter.py`, `snapshotter.py`) deliberately never populate `EnvSnapshot.mission_snapshot` themselves -- by design, production (`ThesisScenarioEnv`) wires it in afterwards via `dataclasses.replace(snapshot, mission_snapshot=self._mission_runtime.snapshot)`. These two synthetic-scenario tests capture snapshots from the adapter directly, against a raw `ScenarioOnlineEnv` with no mission runtime, and never performed that wrap-up step, so `mission_snapshot` stayed at its dataclass default of `None`. Test-fixture-only defect (the production adapter's no-mission-snapshot design is correct and unchanged). Fixed by adding a `_attach_synthetic_mission_snapshot()` test helper that projects the ego position onto the same `EpisodeCache.route_polyline` the transition evaluator itself uses (`RoutePolyline.project(..., previous_s_m=...)` for station continuity across steps), mirroring the production wiring pattern. All 22 tests (20 `test_descriptors_evaluate_one_live_rulebook_transition` parametrizations + 2 `test_vru_collision_descriptors_produce_a_live_collision_onset`) now pass. |
| second root cause found while fixing the above: dead `route_outside_fraction`/`route_adherence` diagnostic (`REQ-EF-15`/M6a) | FIXED | 2026-08-08. Fixing the `s_m` gap above surfaced a distinct, deeper defect: 20 of the 22 tests then failed a *different* assertion (`"route_outside_fraction" in progress.diagnostics`). Investigation found that `evaluate_progress`'s rewrite for the mission v1.1 R4 contract (commit `4ce1233`, 2026-08-04) silently dropped the entire `route_outside_fraction`/`route_adherence` diagnostic (`REQ-EF-15`, M6a, "does not affect any cost or margin" per its own docstring): the helper function `route_outside_fraction()` remained defined in `progress.py` but was never called, and its required inputs (`ego_footprint`, `task_corridor`) were removed from the `"progress"` entry of `component_inputs` in `transition.py` -- `_task_corridor(cache)` (`transition.py:192`) was also left orphaned, called from nowhere. This is diagnostic-only (confirmed by the function's own docstring and by `evaluate_progress`'s cost/margin computation not depending on it), so restoring it does not change any Rulebook cost, reward, or termination behavior -- it is closing an accidental capability loss during the R4 rewrite, not a new scientific decision. Fixed: `evaluate_progress` gained two optional keyword parameters (`ego_footprint`, `task_corridor`, both defaulting to `None` to preserve every existing direct caller's signature, e.g. `tests/test_rulebook_v2_progress.py`); when `ego_footprint` is supplied, it computes and adds `route_outside_fraction`/`route_adherence` to `diagnostics` exactly as before. `transition.py`'s `"progress"` input dict now supplies `"ego_footprint": post_state.ego.footprint` and `"task_corridor": _task_corridor(cache)`, restoring the dropped wiring. All 69 tests across `test_rulebook_synthetic_scenarios.py`, `test_rulebook_v2_progress.py`, and `test_rulebook_v2_transition.py` pass together. Scoped `make lint`/`make format-check` PASS on `progress.py`, `transition.py`, and the test file. |
| root cause and fix, `tests/test_run_metadata.py::test_run_metadata_records_eval_protocol_reproducibility_fields` (1 of the 46) | FIXED | 2026-08-08. Stale test literal, not a production defect: the test hardcoded `metadata["evaluation_protocol"]["version"] == "1.0"`, but `EVALUATION_PROTOCOL_SPECIFICATION_VERSION` in `src/thesis_rl/runtime/io/metadata.py:24` already correctly reads `"1.2"` (the current `AUTHORITATIVE` `EVAL-PROTOCOL` version per `ADR-042`, approved 2026-07-31). Fixed the test literal to `"1.2"` with a comment citing `ADR-042`. Scoped `make lint`/`make format-check` PASS (one unrelated pre-existing format-baseline item in the same file, in a different, untouched function, confirmed via `git stash`, left alone per the no-mass-format policy). |
| investigation of the remaining 9 failures | 8 confirmed pre-existing and out of scope, 1 flagged and paused on explicit user instruction | 2026-08-08. `test_eval_artifacts.py` (4), `test_build_named_panel_manifest_script.py` (2), `test_hydra_preset_run_configs.py::test_run_profile_tune_overrides_experiment_budget` (1), and `test_async_evaluation.py::test_renderables_keep_completed_episode_count_after_job_finishes` (1) were already individually confirmed pre-existing against `scenarionet-implementation` by this session's earlier differential worktree check; each is a distinct, unrelated subsystem (video artifact path templating, a named-panel CLI script's frozen-pool guard, a `run_profile=tune` config default, and Rich-UI async-evaluation status text) and out of scope for this mission ExecPlan per the no-unrelated-cleanup policy. The ninth, `test_thesis_scenario_env.py::test_thesis_reward_suppresses_native_short_route_bonus`, was investigated in depth (see below) and is a candidate genuine regression, not a stale test -- the user was asked how to proceed and chose to pause pending further analysis rather than fix or accept it. |
| `test_thesis_reward_suppresses_native_short_route_bonus` investigation (paused, no code change) | ANALYSIS ONLY, no fix applied | 2026-08-08. Commit `4ce1233` (2026-08-04) removed `reward_function`'s suppression of MetaDrive's native `success_reward` bonus, justified in its docstring as "mission success is tracker-owned" -- true for `done_function`'s `TerminationState.SUCCESS` (correctly now driven exclusively by `mission_snapshot.mission_success`), but that framing does not extend to `reward_function`: MetaDrive's native `ScenarioEnv.reward_function` (`third_party/metadrive/metadrive/envs/scenario_env.py:300-304`, vendored, untouched by the mission rewrite) overwrites `reward` with the flat `success_reward` config value (default `5.0`) on *every step*, not just at termination, whenever its own `_is_arrive_destination(vehicle)` holds -- which is permanently true for the rest of an episode once `vehicle.navigation.reference_trajectory.length < 2` (MetaDrive's own native navigation object, structurally independent of the thesis's mission/`assigned_route_lane_ids` route system; the mission v1.1 rewrite does not touch it). The pre-refactor code detected exactly this divergence (`native_success and not thesis_success`) and substituted the dense `step_reward` in its place. Restoring it is a mechanical, low-risk change (`_is_thesis_success` is unchanged and already called in the current code; the removed comparison only needs `self.agents[vehicle_id]` and `self._is_arrive_destination`, both untouched MetaDrive-native calls) but was intentionally **not applied**: whether this failure mode is empirically reachable on the current frozen catalog is unverified (the M8 audit's route-length finding covers the mission's own `s_goal_m`, a different route object, and does not transfer as evidence here). Left for a follow-up decision/session with the user's explicit direction. |
| `make test` (post-investigation, full suite, third pass) | 1338 passed, 9 failed | 2026-08-08. 37 of the original 46 pre-merge failures fixed this session (14 `test_rulebook_v2_transition.py` + 22 `test_rulebook_synthetic_scenarios.py` + 1 `test_run_metadata.py`). The remaining 9 are exactly the expected set: `test_eval_artifacts.py` (4), `test_build_named_panel_manifest_script.py` (2), `test_hydra_preset_run_configs.py::test_run_profile_tune_overrides_experiment_budget` (1), `test_async_evaluation.py::test_renderables_keep_completed_episode_count_after_job_finishes` (1) -- all independently confirmed pre-existing/unrelated against `scenarionet-implementation` earlier this session -- plus `test_thesis_scenario_env.py::test_thesis_reward_suppresses_native_short_route_bonus`, paused per the analysis above. No unexpected failures. **None of this session's fixes have been committed**; they remain uncommitted working-tree changes pending explicit user instruction. |
| `reward_function` native-bonus suppression: user decision and restoration | RESTORED | 2026-08-08. Follow-up analysis: the pre-merge dataset quality filter (`MINIMUM_SDC_ROUTE_LENGTH_M = 10.0`, `src/thesis_rl/scenarios/quality.py`) reduces but does not provably eliminate this failure mode, because it measures a *different* quantity -- the offline-computed raw SDC recorded-trajectory length (`ego_route = ego_positions[ego_valid]`, `features.py`) -- from the one the native bug actually depends on, MetaDrive's own live-built `vehicle.navigation.reference_trajectory.length` (a separate, live routing/lane-matching object, structurally independent of both the offline SDC track and the thesis mission route). A scenario can in principle pass the >=10 m offline filter while MetaDrive's live navigation object still ends up degenerate. Given (a) the fix is mechanical and low-risk (no new dependency, calls only pre-existing unchanged methods `_is_thesis_success`/`_is_arrive_destination`), (b) the guard is defensive against a vendored MetaDrive quirk this repository does not control, and (c) the cost of the guard is near zero against the cost of a corrupted per-step reward signal for an entire episode, the user explicitly approved restoring it ("vai dai ripristinalo") without requiring further empirical verification of current-catalog incidence. Restored in `src/thesis_rl/envs/thesis_scenario_env.py::reward_function`: substitutes the dense `step_reward` for MetaDrive's flat `success_reward` whenever the native arrival predicate disagrees with the mission-tracker-owned `thesis_success`, restoring `success_reward_suppressed` diagnostics. Full `tests/test_thesis_scenario_env.py` (36/36) and scoped `make lint`/`make format-check` PASS (one unrelated pre-existing format-baseline gap elsewhere in the file, confirmed via `git stash`, left alone per the no-mass-format policy). |
| `make test` (final pass, full suite, after the reward-suppression restoration) | 1339 passed, 8 failed | 2026-08-08. Exactly one fewer failure than the previous pass, confirming the fix and no new regressions. The remaining 8 are `test_eval_artifacts.py` (4), `test_build_named_panel_manifest_script.py` (2), `test_hydra_preset_run_configs.py::test_run_profile_tune_overrides_experiment_budget` (1), `test_async_evaluation.py::test_renderables_keep_completed_episode_count_after_job_finishes` (1) -- all pre-existing and unrelated, independently confirmed against `scenarionet-implementation`. This closes every failure investigated in this session; the remaining 8 are out of scope (distinct, unrelated subsystems) and left for a dedicated follow-up. **Still uncommitted**, pending explicit user instruction. |
| out-of-scope failures: root-cause investigation and fix (separate commits) | 8/8 fixed | 2026-08-08. Per user instruction ("indaga" then "procedi ora, in commit separati dal lavoro sulla mission"), all 8 remaining failures were traced to distinct root causes -- 7 were stale test assertions left behind by commit `0c04fe2` (which changed `eval_artifacts.py`'s final-eval output path to namespace by `scenario_set` for the new named multi-panel evaluation, tightened `build_named_panel_manifest.py`'s `--size` guard to the frozen full-size pool, and doubled `tune.yaml`'s `eval_interval` alongside `long`/`thesis`, all without updating every dependent assertion), and 1 (`test_async_evaluation.py::test_renderables_keep_completed_episode_count_after_job_finishes`) was a genuine production bug (`renderables()` missing the `job.eval_id` fallback already present in the adjacent status-table row). Fixed and committed on this branch, outside this ExecPlan's scope, as `ab8c7de` (async-evaluation fallback bugfix) and `7a4d4f1` (stale-assertion realignment); pushed to `origin/codex/route-coordinate-mission`. `make test` after both commits: 1347 passed, 0 failed. |
