...
| `make smoke` | NOT_RUN | `scenario_catalog_driving_mission_v1_1_1.parquet` is missing on this host (the same pre-existing `pyarrow`-environment artifact-generation gap recorded above, 2026-08-04); follow-up command once provisioned: `make smoke` |
| full `make test` | NOT_REPEATED | same rationale as above; M8 acceptance evidence is the focused suite plus the real-data integration test |
