...
| `docs/specifications/rulebook_v4.12_specification.md` | Created | Extends the `ADR-050` deadband to `wrongway`'s `cost`, not only `status`; the 1e-8 scalarizer tolerance does not catch 0.1 m/s-scale physics noise, so `cost` was already flipping the R3 categorical indicator on a correctly stopped ego under the currently approved `SCAL-V1.0` default |
| `docs/decisions/ADR-056-wrongway-cost-physics-noise-deadband.md` | Created | Supersedes `ADR-050`'s "cost is out of scope" scope conclusion; keeps its noise-magnitude diagnosis |
| `src/thesis_rl/rulebook/v2/components/road.py` | Modified (post-approval follow-up) | `evaluate_wrongway`: `cost` now shares the 0.1 m/s deadband via a continuous reparameterization (`WRONGWAY_SPEED_EPSILON_MPS`, replacing `WRONGWAY_STATUS_SPEED_EPSILON_MPS`); `status` simplified to derive from `cost > 0.0` |
| `tests/test_rulebook_v2_road.py` | Modified (post-approval follow-up) | `test_wrongway_status_has_a_deadband_against_standstill_physics_noise` updated (cost now `== 0.0`, not `> 0.0`); added `test_wrongway_deadband_zeroes_cost_for_standstill_noise`, `test_wrongway_cost_deadband_is_continuous_at_the_boundary`, `test_wrongway_reaches_full_cost_at_speed_cap`, `test_wrongway_status_derives_from_cost_not_a_separate_epsilon`; two pre-existing reverse-motion assertions (`0.5` at `u=5.0, cap=10.0`) updated to `4.9/9.9`, the new deadband-reparameterized value |
