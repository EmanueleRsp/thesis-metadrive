...
- [ ] A4/A5: Complete the scalar S0 vertical path and staged PG/Waymo learner smoke. The remaining blocker is the missing live `rulebook_v2_adapter`.
...
- 2026-07-19: An earlier ScenarioNet learner attempt was blocked by the then-mixed runtime view; canonical replay has since removed that blocker. Rulebook v4.7 still fails closed because `ThesisScenarioEnv` lacks `rulebook_v2_adapter`.
...
- 2026-07-19: Added isolated live-adapter increments in `rulebook/v2/context/metadrive_live.py`: stable ScenarioNet actor IDs, explicit MetaDrive actor taxonomy, finite pose/velocity/footprint normalization, lane identity, speed-cap validation, deterministic vehicle collection, a Bullet contact-onset normalizer/step buffer, and current signal-state extraction through MetaDrive's source/object mapping. Fifteen focused live-adapter tests pass with Ruff; real fixed-sequence PG/Waymo hook probes preserve the vendor callback and ignore three non-actor lane/boundary contacts each, and a real Waymo traffic-light probe returned 14 mapped current states (`RED`/`UNKNOWN`). The increment is not wired into v4.7 training yet; persistent-contact semantics, static cache/zone construction, and the transition evaluator remain required before S1.
- 2026-07-19: Read-only static-adapter probes on actually loaded canonical records produced zero validation errors for one PG record, one Waymo record, and one Waymo record with route traffic lights (the latter yielded two route-relevant signal controls). This validates source conversion on those examples only; live snapshot/evaluator wiring is still not demonstrated.
...
| `tests/test_rulebook_v2_metadrive_live.py` | Added | Regression coverage for stable IDs, taxonomy, dimensions, speed caps, deterministic collection, contact normals/buffering, and current signal states |
...
| `docker compose ... pytest -q tests/test_rulebook_v2_*.py` | PASS | 2026-07-19 | 152 Rulebook v4.7 tests pass, 1 environment-dependent test skipped; live adapter wiring remains a separate blocker. |
| canonical Rulebook v4.7 wiring probe | EXPECTED BLOCK | 2026-07-19 | Canonical ScenarioNet construction succeeds; the explicit gate raises because `ThesisScenarioEnv` has no live `rulebook_v2_adapter`. No learner was started. |
| `docker compose ... pytest -q tests/test_rulebook_v2_metadrive_live.py tests/test_rulebook_v2_live_adapter.py` | PASS | 2026-07-19 | 15 focused tests pass for explicit MetaDrive actor normalization, contact onset normalization/buffering, current signal extraction, and existing live snapshot contracts. |
...
| canonical fixed-sequence PG/Waymo actor/contact hook probe | PASS | 2026-07-19 | One real PG and one real Waymo scenario each produced finite live vehicle snapshots; the vendor callback remained active and three non-actor contacts were ignored per source. Full Rulebook transition evaluation remains unverified. |
| canonical fixed-sequence Waymo live-signal probe | PASS | 2026-07-19 | One real train Waymo scenario with 14 traffic lights produced 14 current states through the ScenarioNet physical-ID mapping; no future `dynamic_map_states` sequence was read. Unknown states remain explicit and full signal transition evaluation remains unverified. |
