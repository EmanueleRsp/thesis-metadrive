...
- [x] M18: add passive per-worker ACL reset timing, including existing
  MetaDrive manager phases and thesis reset-time adapters, without changing
  selection, reset order, scenario reuse, rewards, or observations. A real
  post-episode run is required to identify the dominant measured phase.
- [x] M19: emit the same reset timing immediately as a
  `scenario_acl_reset_timing` event after each ACL reset, instead of waiting
  for the 10k-step chunk summary.
- [x] M20: emit filtered `scenario_acl_slow_step` events immediately for
  worker steps at least one second long, including scenario, ACL episode,
  worker-local step index, and Rulebook timing.
...
| REQ-RP-03, REQ-RP-05--06 | selective ACL reset observability with worker and internal reset phase timings | `tests/test_deterministic_subproc_vec_env.py`; next real ScenarioNet post-episode run | Implemented; real measurement pending |
...
| `src/thesis_rl/envs/thesis_scenario_env.py` | Modified | Emit reset-time breakdown without changing reset behavior. |
| `src/thesis_rl/runtime/execution/deterministic_subproc_vec_env.py` | Modified | Return selective-reset worker timing to the parent. |
| `src/thesis_rl/agent/agent.py` | Modified | Persist reset wall and worker-phase totals in training timing artifacts. |
...
| 2026-07-22 | The observed fast run reaches its first completed episode at 2,436 global transitions, exactly 116 vector iterations with 21 workers; the first completion artifact has episode length 116. This excludes learner updates (`learning_starts=5000`) and locates the pause at the parent-controlled ACL `reset_slots` barrier. Added passive timing for `scenario_selection`, every existing MetaDrive manager `before_reset`/`reset`/`after_reset` call on already initialized engines, route metadata, Rulebook adapter, causal observation setup, base reset observation, worker reset, and parent callback wall time. The timing is emitted only in reset info and the existing internal phase artifact; it does not cache, alter reset scheduling, or alter simulator/Rulebook outputs. `python -m pytest -q tests/test_deterministic_subproc_vec_env.py`: `8 passed in 3.32s`; AST parse and `git diff --check` pass. A real ScenarioNet run past one reset remains required to attribute the pause quantitatively. |
| 2026-07-22 | A dedicated 21-worker, 2,520-transition reset-profile run was started with the diagnostic timing enabled. It remained CPU-bound in initial ScenarioNet bootstrap for more than two minutes (about 12.25 GiB and 281 processes/threads) without a committed transition or traceback, so the owned diagnostic was stopped to release resources. This is consistent with the already observed cold-start/reset bottleneck but cannot attribute manager subphases because the first reset creates the engine; the new detailed manager instrumentation applies once the engine exists, i.e. to the post-episode reset of interest. |
| 2026-07-22 | A real 21-worker LQ-micro + decoder-lite fast run completed its first 10,017-transition chunk with 129 episodes in 1,748.81 s (5.7279 FPS). Reset is not the dominant cause: parent ACL reset callbacks are 53.18 wall-s (3.0% of the chunk) and summed worker reset is 59.06 s, about 0.50 s per completed episode. The largest reset subphases are map destroy/rebuild (1.93 s `before_reset` + 24.51 s `reset`), base initial observation (16.46 s), Rulebook adapter (5.61 s), traffic spawn (4.73 s), and causal observation builder (4.06 s), all summed across resets. In contrast parent synchronous `env.step` is 1,389.74 s (79.5%) and summed Rulebook vehicle-yield is 1,150.57 worker-s (71.4% of 1,611.66 evaluator worker-s). The earlier five-minute apparent pause after the first completion therefore cannot be attributed principally to the reset barrier; it is consistent with subsequent slow synchronous steps in a difficult live scenario, dominated by vehicle-yield. |
| 2026-07-22 | Reset timing is now also appended synchronously to `logs/events.jsonl` as `scenario_acl_reset_timing` immediately after each selective reset, including selection time, parent reset wall time, worker reset timing, and internal reset phases. This removes the need to wait for the 10k-step chunk boundary when diagnosing the next pause. |
| 2026-07-22 | Added a one-second diagnostic threshold for per-step `scenario_acl_slow_step` events. The vector worker reports a local episode step index; the parent records global step, worker, scenario, ACL episode, wrapped environment-step time, and Rulebook subphase timings immediately. Normal steps produce no extra event. AST parsing, the deterministic vector-worker matrix (`8 passed in 3.33s`), and `git diff --check` pass. |
| 2026-07-22 | The first deployment attempt failed before training because `slow_step_callback` was accidentally added to `Agent.train()` instead of `Agent.train_vectorized()`. Moved the keyword to the vectorized signature; AST parsing and `git diff --check` pass. No transitions were affected by this failed attempt. |
| 2026-07-22 | Immediate per-step events identify the reported pause precisely: at global step 2,436, worker 7, ACL episode 21, scenario `3f1985e8163dded1`, one `env.step` takes **284.578 s**, of which Rulebook `vehicle_yield` takes **283.784 s**. The reset immediately before it took only **5.126 s**. Subsequent steps in the same episode remain slow (e.g. 12.58 s vehicle-yield at local step 18 and 8.75 s at local step 42). The reset hypothesis is therefore rejected for this failure; the trigger is a pathological vehicle-yield evaluation in that scenario/transition. |
| 2026-07-22 | Scenario-specific vehicle-yield audit found the expensive cold path in route-zone attachment: `route_interval_for_zone` constructed a GEOS `LineString` and intersection for every segment of every candidate route, including segments whose axis-aligned bounds cannot overlap the buffered conflict zone. Added an exact AABB broad-phase rejection before GEOS calls, preserving the existing epsilon buffering and interval semantics, plus a 1,000-segment regression. Geometry suite: `33 passed`; `git diff --check` passed. |
| 2026-07-22 | A one-step, one-environment reproduction forced `waymo:training_20s:3f1985e8163dded1`, with ACL and vectorization disabled, still took **281.02 s**. The failure is therefore intrinsic to Rulebook geometry rather than reset, ACL, or the vector barrier. The residual pathological path is deterministic ear clipping, whose repeated all-vertex search is cubic for a large derived conflict-zone boundary. For polygons above 64 boundary vertices, use the already coverage-checked GEOS constrained triangulation path; the polygonal occupancy domain is unchanged. The same forced reproduction completes its single reset-plus-step training chunk in **about 10 s**, a roughly **28x** end-to-end improvement. The subsequent asynchronous evaluation fails because the one-scenario training manifest is intentionally absent from the test catalog; it occurs after the measured step and is not a Rulebook failure. Focused geometry/collision suite: `41 passed`; `git diff --check` passed. |
| 2026-07-22 | Added two further exact geometric broad phases: canonical corridor AABB rejection before a vehicle conflict-zone boolean intersection, and conservative constant-velocity swept-AABB rejection before each occupancy SAT comparison (both at whole-zone and convex-component level). They can only omit pairs that cannot intersect anywhere in the prediction horizon; no geometry, interval, reward, or Rulebook applicability is changed. Focused geometry/collision suite: `43 passed`; `git diff --check` passed. |
| 2026-07-22 | The forced one-step reproduction after both broad phases completes reset plus evaluation in about **9 s**, preserving the removal of the pathological 281 s pause. The expected post-step diagnostic-only asynchronous-evaluation catalog error remains outside the measured transition. |
| 2026-07-23 | All three algorithm runs fail in the same live contact path: `ScenarioTrafficLight` is excluded from the actor registry as intended, but Bullet contact callbacks still pass it to `_actor_class`, which raised `ValueError` and killed the vector worker. Classified traffic-light contact objects as `STATIC_COLLIDABLE` for collision onset records only; they remain excluded from live actor snapshots and signal state remains owned by the dedicated signal provider. Added a deterministic contact regression. Focused live-adapter/collision suite: `35 passed`; `git diff --check` passed. |
