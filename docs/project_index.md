...
| Transition-level replay v1 | `specifications/transition_replay_v1_specification.md`; ID `TRANSITION-REPLAY`, version `1.0` | `AUTHORITATIVE`; explicit user approval 2026-07-17 | `implementation/transition_replay_v1.0_exec_plan.md`; `IN_PROGRESS`; configuration, N-step, canonical collection, PER, persistence, and reward compatibility implemented | Focused replay matrix passes; full suite has five unrelated Hydra preset failures and smoke is blocked by missing Rulebook v2 adapter |
...
| Scalar autonomous-driving pipeline audit and completion | Authoritative ScenarioNet v1.1, ACL v1 §28, Rulebook v4.7, scalarization v1.0, observation v1.1, encoder v1.0, and transition replay v1.0 | `implementation/scalar_autonomous_driving_pipeline_audit_exec_plan.md` | `IN_PROGRESS`; `METADATA AUDIT COMPLETE — LIVE DATASET VALIDATION PENDING`; 48-reference content validation, raw PG/Waymo S0, vectorized PG/Waymo reset, standard GPU checkpoint smoke, and canonical frozen-index reconstruction pass; live Rulebook adapter blocks full learner stages; ACL usefulness amendment approved; transition replay remains v1/n=3 | 2026-07-19 |
