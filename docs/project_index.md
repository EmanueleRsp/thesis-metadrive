...
| Transition-level replay v1 | `specifications/transition_replay_v1_specification.md`; ID `TRANSITION-REPLAY`, version `1.0` | `AUTHORITATIVE`; explicit user approval 2026-07-17 | `implementation/transition_replay_v1.0_exec_plan.md`; `IN_PROGRESS`; configuration, N-step, canonical collection, PER, persistence, and reward compatibility implemented | Focused replay matrix and current Hydra preset matrix pass; learner smoke is blocked by missing Rulebook v2 adapter |
