...
| Scalar autonomous-driving pipeline audit and completion | Authoritative ScenarioNet v1.1, ACL v1 §28, Rulebook v4.7, scalarization v1.0, observation v1.1, encoder v1.0, and transition replay v1.0 | `implementation/scalar_autonomous_driving_pipeline_audit_exec_plan.md` | `IN_PROGRESS`; `METADATA AUDIT COMPLETE — LIVE DATASET VALIDATION PENDING`; 48-reference content validation, raw PG/Waymo S0, vectorized PG/Waymo reset, standard GPU checkpoint smoke, canonical frozen-index reconstruction, persistent live-contact semantics, topology-preserving static adapters, immutable Rulebook cache construction, complete registry transition composition, deferred live-adapter installation, diagnostic S0–S6 source-backed smokes, and a 200-step GPU S5 plus 100-step checkpoint-resume diagnostic pass; dataset-wide live validation, broader semantic/checkpoint-resume coverage, and thesis experiments remain pending; ACL usefulness amendment approved; transition replay remains v1/n=3 | 2026-07-19 |
