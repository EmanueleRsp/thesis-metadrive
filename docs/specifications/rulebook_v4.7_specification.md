---
title: "Final Rulebook v4.7 specification for MetaDrive ScenarioEnv and ScenarioNet"
subtitle: "Causal filtered CTRV for vehicle conflict-zone occupancy"
...
status: "approved"
authoritative: "YES"
base_specification: "docs/specifications/rulebook_v4.6_specification.md"
base_version: "4.6-final-implementation-complete"
related_amendment: "incoming/archive/rulebook_v4.7_ctrv_amendment_UNDER_REVIEW.md"
related_adr: "docs/decisions/ADR-003-causal-ctrv-conflict-zone-prediction.md"
decision_approval: "2026-07-17"
approval_confirmed: "2026-07-17"
---

# Review status

This specification is authoritative as of the explicit user approval recorded
on 2026-07-17. It supersedes the v4.6 Rulebook contract for the selected
...
- causal memory includes collision/timer/crossing/signal state plus bounded per-vehicle heading history used only for vehicle conflict-zone occupancy; route progress uses a deterministic wrapper-owned curvilinear coordinate;
...
- lane association, longitudinal gap, drivable surface, traffic controls, control line, route projection, TTC interval, and class-dependent conflict-zone occupancy interval are canonical primitives with one specified algorithm each.
...
rulebook:
  ...
  prediction:
    ...
    conflict_zone_occupancy:
      vehicle_motion_model: causal_ctrv_ols
      non_vehicle_motion_model: constant_velocity_constant_heading_no_rotation
      history_window_s: 0.5
      minimum_history_samples: 3
      stationary_speed_epsilon_mps: 0.1
      yaw_rate_straight_epsilon_rad_s: 1.0e-3
      rotating_solver: deterministic_uniform_sweep_with_bisection
      rotating_occupancy_max_step_s: 0.02
      interval_selection: containing_zero_else_earliest
...
# 17.1 Integrated CTRV replacement contract

This section replaces every v4.6 statement that requires vehicle
conflict-zone occupancy to use constant world-frame velocity, fixed heading,
non-rotation, or continuous SAT. Those v4.6 statements remain applicable
solely to TTC and pedestrian/cyclist conflict-zone occupancy. Rule hierarchy,
costs, candidates, zone geometry/identity/cache, priority predicates, learner
interface, policy observation, termination, and truncation are unchanged.

### 17.1.1 Causal history and model separation

TTC exclusively retains v4.6 exact CV/non-rotating continuous SAT. For
crosswalk and vehicle-yield zones only, ego and live VEHICLE actors use causal
filtered CTRV with sufficient valid history; otherwise they use exact CV
straight solving. Pedestrians/cyclists remain CV/non-rotating.

RulebookMemory gains immutable ActorMotionSample(timestamp_s, position_xy_m,
heading_rad, velocity_xy_mps), ActorMotionHistory(actor_id, samples), and
actor_motion_histories. Reset clears history then inserts one ego/live-vehicle
sample. One central writer creates a trimmed [t-T_h,t] post-state preview,
removes absent actors, and commits exactly one history delta atomically with
memory/cache after successful evaluation. Crosswalk and vehicle-yield read the
same preview. A reappearing ID after absence begins a new history. Terminal or
truncated transitions evaluate normally and the next reset clears histories.

Each continuous actor history requires finite strictly increasing sim_time_s.
Duplicate/decreasing time, conflicting current actor records, non-finite
kinematics/footprint/prediction, or multiple history writers raise
RulebookEvaluationError. Short history, stationary motion, and straight-limit
estimates are logged fallbacks, not NOT_EVALUABLE.

### 17.1.2 CTRV propagation and rotating interval

Headings are causally unwrapped and yaw rate is OLS against actual timestamps:

\[
\widehat\omega_t =
\frac{\sum_k(t_k-\bar t)(\widetilde\psi_k-\overline{\widetilde\psi})}
{\sum_k(t_k-\bar t)^2}.
\]

Use signed heading-aligned speed
\(v_t=\mathbf v_t^\top[\cos\psi_t,\sin\psi_t]^\top\). If it is within
\(\varepsilon_v\), used speed/yaw are zero. With
\(|\widehat\omega_t|>\varepsilon_\omega\), propagate:

\[
x(\tau)=x_t+\frac{v_t}{\widehat\omega_t}
[\sin(\psi_t+\widehat\omega_t\tau)-\sin\psi_t],
\]
\[
y(\tau)=y_t+\frac{v_t}{\widehat\omega_t}
[-\cos(\psi_t+\widehat\omega_t\tau)+\cos\psi_t],\quad
\psi(\tau)=\operatorname{wrap}_{(-\pi,\pi]}(\psi_t+\widehat\omega_t\tau).
\]

Translate and rotate the current local footprint; negative signed speed is
valid reverse motion. Insufficient history or the straight limit reuses exact
v4.6 CV continuous SAT.

For active CTRV, form a deterministic uniform grid containing zero/horizon
with spacing at most \(\Delta t_{\mathrm{sweep}}\); test polygon overlap;
bisect false-to-true/true-to-false brackets to \(\varepsilon_t\); use zero
entry if inside; use symbolic OPEN_END at horizon overlap; merge gaps at most
\(\varepsilon_t\); then choose containing-zero or earliest. This is
deterministic but approximate and can miss events shorter than sweep resolution.
Each vehicle query logs actor ID, history count/span, CTRV/straight/fallback
model, raw/used yaw, signed speed, horizon, sweep step, and result; never to
the policy.

### 17.1.3 Frozen engineering defaults and remaining repository gates

Frozen required values: horizon 3.0 s; TTC CV/non-rotating continuous SAT;
vehicle zone causal_ctrv_ols; non-vehicle zone unchanged CV; history window
0.5 s; minimum samples 3; stationary epsilon 0.1 m/s (semantically distinct
from the stop threshold); straight yaw epsilon 1e-3 rad/s; deterministic
sweep+bisection; maximum sweep step 0.02 s; exact v4.6 continuous-SAT reuse
for straight motion and fallback; reset history after any absence; strictly
increasing sim_time_s. These are finite, run-manifest serialized, a-priori
project engineering defaults. They must not be calibrated or selected through
training performance, dataset-wide search, sensitivity analysis, or
v4.6/v4.7 ablation.

| ID | Material decision | Status |
|---|---|---|
| DEC-CTRV-001 | history window 0.5 s | ACCEPTED — user approval 2026-07-17 |
| DEC-CTRV-002 | minimum OLS samples 3 | ACCEPTED — user approval 2026-07-17 |
| DEC-CTRV-003 | stationary 0.1 m/s and straight 1e-3 rad/s | ACCEPTED — user approval 2026-07-17 |
| DEC-CTRV-004 | maximum sweep step 0.02 s | ACCEPTED — user approval 2026-07-17; conformance oracle required |
| DEC-CTRV-005 | clean exact-CV reuse in straight cases | ACCEPTED — user approval 2026-07-17 |

### 17.1.4 Tests, compatibility, and repository gates

The mandatory CTRV validation scope is implementation conformance only:

1. unit and boundary tests for OLS, heading unwrap, CTRV propagation, and CV
   fallback;
2. history lifecycle, reset, disappearance/ID reuse, and timestamp monotonicity;
3. instrumented guards proving no future track, route, or maneuver access;
4. one synthetic comparison of the 0.02 s sweep with a substantially finer
   numerical oracle, checking missed relevant events and entry/exit tolerance;
5. a small fixed PG/Waymo integration smoke suite covering straight and curved
   conflicts;
6. deterministic replay, finite bounded outputs, and exact v4.6 equality in the
   straight branch.

These are conformance tests, not scientific experiments. Dataset-wide
calibration, learned-policy performance validation, path-conditioned
prediction comparison, CTRV-vs-CV ablations, sensitivity analysis, and
training runs for parameter selection are explicitly NOT REQUIRED and must not
be used to choose these frozen values.

v4.6 in-episode RulebookMemory serialization is incompatible and has no
migration; restart at reset. Neural checkpoints may load structurally, but runs
must not continue across rulebook versions and curved-zone results must not be
pooled. Repository inspection verifies finite sim_time_s, per-snapshot unique
IDs, heading, world-XY velocity, OBB footprint, and separable CV continuous
SAT. It does not prove timestamp monotonicity, continuous identity/no reuse,
PG/Waymo live providers, causality guards, or full occupancy wiring; these
remain implementation-conformance gates.
