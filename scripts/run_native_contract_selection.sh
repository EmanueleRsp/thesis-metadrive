...
usage() {
  cat <<'EOF'
Usage:
  scripts/run_native_contract_selection.sh run [options]
  scripts/run_native_contract_selection.sh run-parallel [options]
  scripts/run_native_contract_selection.sh cleanup [options]
  scripts/run_native_contract_selection.sh launch [options]
...
Examples:
  ...
  scripts/run_native_contract_selection.sh cleanup --tag v1
  scripts/run_native_contract_selection.sh analyze --tag v1

Options:
  --variants LIST         Comma-separated list from: strict,relaxed
  ...
  --run-profile NAME      Analysis run-profile label. Default: fast
...
collect_variants() {
  ...
  [[ ${#variants[@]} -gt 0 ]] || die "no variants selected"
  local variant
  for variant in "${variants[@]}"; do
    case "$variant" in
      strict|relaxed) ;;
      *)
        die "unsupported variant '$variant' (expected strict or relaxed)"
        ;;
    esac
  done
}
...
session_name_for() {
  local variant="$1"
  printf 'native_contract_%s_sac_fast_%s' "$variant" "$tag"
}

experiment_group_for() {
  local variant="$1"
  printf 'EXP_native_contract_%s_sac_fast_%s' "$variant" "$tag"
}

preset_for() {
  local variant="$1"
  printf 'presets/selection/sac_sb3_native_contract_%s_fast' "$variant"
}
...
launch_runs() {
  ...
  for variant in "${variants[@]}"; do
    ...
    local preset_name
    session_name="$(session_name_for "$variant")"
    experiment_group="$(experiment_group_for "$variant")"
    preset_name="$(preset_for "$variant")"

    log "launching group for variant=$variant"
  ...
  echo "  analyze with: scripts/run_native_contract_selection.sh analyze --tag $tag"
}
...
while true; do
  ...
  for ((seed = seed_start; seed <= seed_end; seed++)); do
    seed_root="$OUT_ROOT/$group/sac_sb3/seed_${seed}"
    status="pending"
  ...
  printf '[%s] [wait][%s] completed=%d/%d pending=%d running=%d failed=%d interrupted=%d\n' \
    "$now" "$group" "$completed" "$expected_count" "$pending" "$running" "$failed" "$interrupted"
  printf '[%s] [wait][%s] %s\n' "$now" "$group" "${details[*]}"
  ...
  if has_live_train_process; then
    stale_polls=0
    printf '[%s] [wait][%s] live training process detected\n' "$now" "$group"
  else
    stale_polls=$((stale_polls + 1))
    printf '[%s] [wait][%s] no live training process detected (stale poll %d)\n' \
      "$now" "$group" "$stale_polls"
...
run_all() {
  ...
  for variant in "${variants[@]}"; do
    ...
    log "starting sequential run for variant=$variant"
    "$repo_root/scripts/tmux_seed_grid.sh" \
      --session "$session_name" \
      --window "$window_name" \
      --seed-start "$seed_start" \
    ...
    log "waiting for variant=$variant to complete (poll every ${poll_seconds}s)"
    wait_for_group "$variant" "$experiment_group"
    log "variant=$variant completed successfully"
  done

  log "all native contract groups completed, starting analysis"
  analyze_runs
}
...
run_all_parallel() {
  ...
  log "launching all requested native contract groups in parallel"
  launch_runs
  ...
  log "all parallel native contract groups completed, starting analysis"
  analyze_runs
}
...
analyze_runs() {
  ...
  local variant
  for variant in "${variants[@]}"; do
    groups+=("$(experiment_group_for "$variant")")
  done

  log "starting native contract analysis for tag=$tag on variants=${variants_csv}"
...
FINAL_TABLE="$ANALYSIS_ROOT/$run_profile/comparisons/task_contract/sac_sb3__native__monitor_only__disabled__selection/tables/final_evaluation.csv"
echo
echo "Final table:"
echo "  $FINAL_TABLE"
echo
...
main() {
  ...
  case "$subcommand" in
    run)
      log "mode=run tag=$tag variants=$variants_csv seeds=${seed_start}..${seed_end} poll=${poll_seconds}s"
      run_all
      ;;
    run-parallel)
      log "mode=run-parallel tag=$tag variants=$variants_csv seeds=${seed_start}..${seed_end} poll=${poll_seconds}s"
      run_all_parallel
      ;;
    cleanup)
      log "mode=cleanup tag=$tag variants=$variants_csv"
      cleanup_sessions
      ;;
    launch)
      log "mode=launch tag=$tag variants=$variants_csv seeds=${seed_start}..${seed_end}"
      launch_runs
      ;;
    analyze)
      log "mode=analyze tag=$tag variants=$variants_csv seeds=${seed_start}..${seed_end}"
      analyze_runs
      ;;
  esac
}
