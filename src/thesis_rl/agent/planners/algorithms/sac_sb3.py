...
def _require_sb3_sac():
    try:
        ...
        raise ImportError(
            "The `sac_sb3` backend requires `stable-baselines3` to be installed. "
...
class Sb3SacPlannerBackend(BasePlannerBackend):
    ...
    def _build_action_noise(env: Any, cfg_planner: dict[str, Any]):
        _SAC, NormalActionNoise, VectorizedActionNoise, VecEnv = _require_sb3_sac()
        del _SAC
        ...
        if action_space is None or not hasattr(action_space, "shape"):
            raise ValueError(
                "SAC normal action noise requires an environment with a shaped action space."
            )
    ...
    def _resolve_gradient_steps(env: Any, cfg_planner: dict[str, Any]) -> int:
        _SAC, _NormalActionNoise, _VectorizedActionNoise, VecEnv = _require_sb3_sac()
        del _SAC, _NormalActionNoise, _VectorizedActionNoise
    ...
    def _validate_sb3_components(self) -> None:
        ...
        if policy is None:
            raise AttributeError(
                "SB3 SAC model is missing `policy`; cannot run lifecycle training."
            )
        if not hasattr(policy, "scale_action") or not hasattr(policy, "unscale_action"):
            raise AttributeError(
                "SB3 SAC policy must expose `scale_action` and `unscale_action` "
        ...
        if action_noise is not None:
            if not callable(action_noise):
                raise TypeError("SB3 SAC `action_noise` must be callable when provided.")
            if not hasattr(action_noise, "reset"):
                raise AttributeError("SB3 SAC `action_noise` must expose `reset()`.")
    ...
    def _validate_train_freq(self) -> None:
        ...
        if train_freq is None:
            raise AttributeError("SB3 SAC model is missing `train_freq`.")
        if not hasattr(train_freq, "frequency") or not hasattr(train_freq, "unit"):
            raise AttributeError("SB3 SAC `train_freq` must expose `frequency` and `unit`.")
        if int(train_freq.frequency) <= 0:
            raise ValueError("SB3 SAC `train_freq.frequency` must be > 0.")

        unit_name = str(train_freq.unit).lower()
        if "step" not in unit_name:
            raise ValueError(
                "Current SB3 SAC lifecycle supports only step-based `train_freq`. "
    ...
    def build(
        ...
        model = SAC(
            ...
            buffer_size=int(planner_cfg.get("buffer_size", 1_000_000)),
            ...
            ent_coef=planner_cfg.get("ent_coef", "auto"),
            target_update_interval=int(planner_cfg.get("target_update_interval", 1)),
            target_entropy=planner_cfg.get("target_entropy", "auto"),
    ...
    def load(
        ...
        model = SAC.load(str(normalize_checkpoint_path(checkpoint_path)), env=env, device=device)
    ...
    def act_train(self, observation: np.ndarray, deterministic: bool = False) -> np.ndarray:
        if self._policy is None:
            raise RuntimeError("SB3 SAC backend not initialized; call begin_training() first.")

        if deterministic:
            action, _ = self.model.predict(observation, deterministic=True)
            action_np = np.asarray(action, dtype=np.float32)
            self._last_buffer_actions = np.asarray(
                self._policy.scale_action(action_np),
                dtype=np.float32,
            )[None, ...]
            return action_np
    ...
    def to_buffer_action(self, env_action: np.ndarray) -> np.ndarray:
        if self._last_buffer_actions is not None:
            ...
            if cached.ndim == 2 and cached.shape[0] > 0:
                return cached[0]
        if self._policy is None:
            raise RuntimeError("SB3 SAC backend policy unavailable; call begin_training() first.")
