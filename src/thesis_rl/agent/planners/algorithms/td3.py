...
class Td3PlannerBackend(BasePlannerBackend):
    ...
    def collection_learning_potential_batch(
        ...
        """Compute TD3 residuals on the transitions collected in this batch."""
        ...
        with torch.no_grad():
            next_actions = torch.clamp(self.actor_target(next_obs), -1.0, 1.0)
            target_q1, target_q2 = self.critic_target(next_obs, next_actions)
            target = reward_tensor + (1.0 - done_tensor) * self.gamma * torch.minimum(
                target_q1, target_q2
            )
