    ...
    try:
        while current_global_step < total_timesteps:
            ...
            recorder.append_row(
                "train_chunks.csv",
                {
                    **base_csv_fields,
                    ...
                    "ep_rew_std": float(summary.get("ep_rew_std", 0.0)),
                    "ep_rew_ci_95": float(summary.get("ep_rew_ci_95", 0.0)),
                    "ep_env_rew_mean": summary.get("ep_env_rew_mean"),
                    "ep_scalar_rule_rew_mean": summary.get("ep_scalar_rule_rew_mean"),
                    "ep_hybrid_rew_mean": summary.get("ep_hybrid_rew_mean"),
                    "ep_len_mean": float(summary.get("ep_len_mean", 0.0)),
                    "ep_len_std": float(summary.get("ep_len_std", 0.0)),
                    "ep_len_ci_95": float(summary.get("ep_len_ci_95", 0.0)),
                    "ep_success_rate": summary.get("ep_success_rate"),
                    "ep_collision_rate": summary.get("ep_collision_rate"),
                    "ep_out_of_road_rate": summary.get("ep_out_of_road_rate"),
                    "ep_route_completion_mean": summary.get("ep_route_completion_mean"),
                    "actor_loss": float(summary.get("actor_loss", float("nan"))),
                    "critic_loss": float(summary.get("critic_loss", float("nan"))),
                    "actor_loss_ema": summary.get("actor_loss_ema"),
                    "critic_loss_ema": summary.get("critic_loss_ema"),
                    "learning_rate": float(summary.get("learning_rate", float("nan"))),
                    "update_calls": int(summary.get("update_calls", 0)),
                    "n_updates": int(summary.get("n_updates", 0)),
                    "fps": float(summary.get("fps", 0.0)),
                    "elapsed_seconds": float(summary.get("elapsed_seconds", 0.0)),
                    "train_reset_seed_first": summary.get("train_reset_seed_first"),
                    "train_reset_seed_last": summary.get("train_reset_seed_last"),
                    "train_reset_seed_unique_count": summary.get("train_reset_seed_unique_count"),
            ...
            history_payload = {
                ...
                "training_phase_seconds": summary.get("phase_seconds", {}),
                "environment_timing_seconds": (collect_scenario_runtime_stats(env) or {}).get(
                    "timing_seconds", {}
                ),
