...
class CausalSemanticBatchBuilder:
    ...
    def _dynamic_key(
        ...
        ahead = 0 if self._route_s(actor, ego) >= self._route_s(ego, ego) else 1
        return (1, relation, ahead, abs(self._route_lateral(actor)), distance, actor.actor_id)
    ...
    def _dynamic_features(
        ...
        actor_cap = actor.configured_speed_cap_mps or max(
            ego_speed_cap, hypot(*actor.velocity_xy), 1.0
        )
