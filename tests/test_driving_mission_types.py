...
def test_mission_record_hash_is_deterministic_and_round_trips() -> None:
    ...
    record = DrivingMissionRecord("uid", "mission-builder-v1", (MissionSection("section:0", span, (span,), gate),), gate)

    assert record.mission_hash == DrivingMissionRecord.from_dict(record.to_dict()).mission_hash
    assert record.mission_hash == DrivingMissionRecord("uid", "mission-builder-v1", (MissionSection("section:0", span, (span,), gate),), gate).mission_hash
