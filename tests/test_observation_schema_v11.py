...
def _batch() -> SemanticObservationBatch:
    ...
    for name, shape in schema.group_shapes.items():
        ...
        value += size
    return SemanticObservationBatch(**arrays)
...
def test_v11_schema_numpy_round_trip_preserves_all_groups() -> None:
    schema = SemanticObservationSchemaV11()
    batch = _batch()
    ...
    assert flat.shape == (2541,)
    for name in schema.group_shapes:
        np.testing.assert_array_equal(getattr(restored, name), getattr(batch, name))


def test_v11_schema_torch_unflatten_preserves_autograd_dtype_and_device() -> None:
    schema = SemanticObservationSchemaV11()
    flat = torch.arange(2 * schema.flat_dim, dtype=torch.float32).reshape(2, -1)
    flat.requires_grad_()

    structured = schema.unflatten_torch(flat)
    loss = structured.dynamic.sum() + structured.interactions.sum()
    loss.backward()

    assert structured.dynamic.dtype is torch.float32
    assert structured.dynamic.device == flat.device
    assert flat.grad is not None
    assert torch.isfinite(flat.grad).all()


def test_v11_schema_fingerprint_is_deterministic_and_sensitive_to_contract() -> None:
    schema = SemanticObservationSchemaV11()

    assert schema.fingerprint_sha256() == SemanticObservationSchemaV11().fingerprint_sha256()
